...
def register():
    ...
    if form.validate_on_submit():
        ...
        try:
            lowerUser = form.user_name.data.lower().strip()
            services.add_user(lowerUser, form.password.data, repo.repository_instance)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # user = services.get_user(form.user_name.data, repo.repository_instance)
        # print("Login Details")
        ...
        try:
            ...
            session.clear()
            print("adding session cookie below in auth.py")
            # print("SESSION DETAIL: ", session['user_name'], " FETCHED DETAIL ", user['user_name'])
